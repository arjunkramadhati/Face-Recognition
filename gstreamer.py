...
def run_pipeline(pipeline, layout, loop, render_overlay, display, handle_sigint=True, signals=None):
    ...
    print(pipeline)
    print("-----------")
    """v4l2src device=/dev/video1 ! video/x-raw,height=480,format=YUY2,width=640,framerate=30/1 ! tee name=t
t. ! queue max-size-buffers=1 leaky=downstream ! videoconvert ! x264enc threads=4 aud=False tune=zerolatency speed-preset=ultrafast bitrate=1000 key-int-max=5 ! video/x-h264,profile=baseline ! h264parse ! video/x-h264,alignment=nal,stream-format=byte-stream ! appsink drop=False emit-signals=True sync=False max-buffers=1 name=h264sink
t. ! queue ! glfilterbin filter=glcolorscale ! video/x-raw,height=225,format=RGBA,width=300 ! videoconvert ! video/x-raw,height=225,format=RGB,width=300 ! videobox autocrop=True ! video/x-raw,height=300,width=300 ! appsink drop=True emit-signals=True sync=False max-buffers=1 name=appsink
     """
    pipeline = ("""rtspsrc location=rtsp://admin:senz25118@192.168.123.169 latency=0 ! queue ! application/x-rtp,encoding--name=h264,media=video,framerate=10,width=640,height=480 ! rtph264depay ! avdec_h264 ! tee name=t
