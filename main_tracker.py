...
from mtcnn_detect_custom import MTCNNDetect_custom
...
t. ! queue max-size-buffers=1 leaky=downstream ! videoconvert ! x264enc threads=4 aud=False tune=zerolatency speed-preset=ultrafast bitrate=1000 key-int-max=5 ! video/x-h264,profile=baseline ! h264parse ! video/x-h264,alignment=nal,stream-format=byte-stream ! appsink sync=False drop=False max-buffers=1 emit-signals=True name=h264sink
t. ! queue max-size-buffers=1 ! decodebin ! glfilterbin filter=glcolorscale ! video/x-raw,format=RGBA,height=480,width=640 ! videoconvert ! video/x-raw,format=RGB,height=480,width=640 ! videobox autocrop=True ! video/x-raw,height=480,width=640 ! appsink sync=False drop=True max-buffers=1 emit-signals=True name=appsink"""'''
...
t. ! queue max-size-buffers=1 ! decodebin ! glfilterbin filter=glcolorscale ! video/x-raw,format=RGBA,height=480,width=640 ! videoconvert ! video/x-raw,format=RGB,height=480,width=640 ! videobox autocrop=True ! video/x-raw,height=480,width=640 ! appsink sync=False drop=True max-buffers=1 emit-signals=True name=appsink"""
        ...
        if len(self.trackers)>0:
            ...
            for (label,tracker) in zip(self.labels, self.trackers):
                ...
                if update <4 :
                    ...
                    #self.labeltracker.remove(self.labels.index(label))
            ...
            print("Tracking")    
            self.render_overlay(overlay("Sigsenz FaceRec Algorithm",objs,get_color,0.1,0.2,self._layout))
        
        
        if self.counter%45==0 or len(self.trackers)==0 and self.isRecognisingSecond == False:
            print("detecting")
            self.trackers=[]
            self.labels=[]
            self.counter=0
            ...
            if not self.isRecognising:
                ...
                thread1=threading.Thread(target=self.recog_face,args=(frame,))
                thread1.start()
            #print("recognized")
            #print(frame.shape)

        if len(self.labeltracker)>0 and self.isRecognisingSecond == True and self.isRecognising == False:
            print("enter 1")
            self.countertwo=self.countertwo+1
            #if self.sleeping == False and self.isRecognisingSecond == False:
                #thread_sleeper=threading.Thread(target=self.sleeper,args=(0.5,))
                #thread_sleeper.start()
                #self.sleeping=True
                
            if self.sleeping == False and self.countertwo%25==0:
                print("enter 2")
                self.countertwo=0
                self.isRecognisingSecond = True
                for i in self.labeltracker:
                    #print(i)
                    t=self.trackers[i]
                    current_pos=t.get_position()
                    self.reclist.append([int(current_pos.left()),int(current_pos.top()),int(current_pos.right()),int(current_pos.bottom())])
                    self.uklabels.append(self.labels[i][1])
                thread_recog_secondtry=threading.Thread(target=self.recog_face_secondtry, args=(frame,self.reclist,self.uklabels,))
                thread_recog_secondtry.start()
                #print(self.reclist)
                #print(self.uklabels)
                self.reclist=[]
                self.uklabels=[]
                self.labeltrackercopy=self.labeltracker
                #self.isRecognisingSecond = True
    ...
    def recog_face_secondtry(self,frame,reclist,uklabels):
        #print(reclist)
        #print(uklabels)
        landmarks= face_detect_second.landmarks(frame,predictor,reclist)
        aligns=[]
        positions=[]
        ...
        #print(landmarks)
        for i in self.labeltracker:
            #print("Changing ", self.labels[i][0], " to ", recog_data[i][0])
            #print(recog_data[i][:])
            self.labels[i]=recog_data[i][:]
        print("here")
        print(recog_data)
        self.labeltracker=[]
        self.isRecognisingSecond = False
        ...
        if(len(aligns) > 0):
            ...
            for (i,rect) in enumerate(rects):
                ...
                if recog_data[i][0] !="Unknown":
                    if recog_data[i][0] in uids:
                        print("Detected unknown: Trying to redetect...")
                        self.labeltracker.append(i)
                    ...
                    if recog_data[i][0] not in uids:
                        print("Detected: ", recog_data[i][0])
                        print(now.strftime("%Y-%m-%d %H:%M:%S"))
                        self.render_face(recog_data[i][0],now.strftime("%Y-%m-%d %H:%M:%S"))
...
def camera_recog():
    ...
    with StreamingServer(video, 10000) as server:
        def render_overlay(overlay):
            ...
            #pass
        def render_face(name,ts):
            #print("render face ready")
            server.send_face(name,ts)

        video.render_overlay = render_overlay
        video.render_face=render_face
        signal.pause()
...
if __name__ == '__main__':
    ...
    face_detect_second=MTCNNDetect_custom();
    main(args);
