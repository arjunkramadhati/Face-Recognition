...
def camera_recog():
    ...
    #vs=cv2.VideoCapture("rtsp://admin:senz25118@192.168.123.169")
    #vout=cv2.VideoWriter("/home/vijayalaxmi/Documents/Face Recognition/FaceRec-master/output.avi",0,10,((int)(vs.get(cv2.CAP_PROP_FRAME_WIDTH)),(int)(vs.get(cv2.CAP_PROP_FRAME_HEIGHT))))
    detect_time = time.time()
    while True:
        _,frame = vs.read();
        #u can certainly add a roi here but for the sake of a demo i'll just leave it as simple as this
        rects, landmarks = face_detect.detect_face(frame,15);#min face size is set to 80x80
        aligns = []
        positions = []
        #print("rects")
        #print(rects)
