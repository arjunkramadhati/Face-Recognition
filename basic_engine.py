...
from edgetpu_cpp_wrapper import BasicEnginePythonWrapper
...
class BasicEngine(object):
  ...
  def get_input_tensor_shape(self):
    ...
    [1, height, width, channels]. To be used as input for :func:`RunInference`,
  ...
  def get_all_output_tensors_sizes(self):
    ...
    A model may output several tensors, but the return from :func:`RunInference`
  ...
  def total_output_array_size(self):
    """Gets the expected size of the 1-D output array returned by :func:`RunInference` and
    :func:`get_raw_output`.
  ...
  def required_input_array_size(self):
    """Gets the required size for the ``input_tensor`` given to :func:`RunInference`.
  ...
  def get_raw_output(self):
    ...
    Returns:
      ...
      array. (Same as what's returned by :func:`RunInference`.)
    """
